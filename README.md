...
- 十个主状态：待机、工作、思考、出错、完成、睡觉、钓鱼、敲木鱼、十万个为什么、童话故事。
- 新增伸懒腰、打哈欠、吃小鱼干、偷看用户四个自然待机动作。
- 摸鱼时敲木鱼连续循环、动作与音效严格同步，每击弹出「功德 +1」；钓鱼每 5–10 秒上一条鱼并弹「食物 +1」。本地木鱼声音默认开启（可在设置关闭），默认音量为 25%。
- 手机版是可添加到主屏幕的 PWA，共用角色状态和声音，但不能跨应用悬浮。

## 无安装预览

项目继续使用原生 HTML、CSS 和 JavaScript，不需要前端框架。不要直接双击 `src/index.html`：`file://` 会拦截模块脚本，页面会显示但没有动作。电脑已有 Python 时，双击项目根目录的 `预览V2.vbs` 即可无终端启动本地预览；也可以手动运行：
...
请双击项目根目录的 `预览V2.vbs`（后台启动本地服务），或双击 `打开网页.cmd`。它们会自动打开可交互网页。
